...
# Linking rules using object files from $(OBJDIR)
...
# Compilation rules for shared source files in common/
...
$(patsubst %, $(OBJDIR)/%_aim2.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_aim2.o: src/%/aim2.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

$(patsubst %, $(OBJDIR)/%_field.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_field.o: src/%/field.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

$(patsubst %, $(OBJDIR)/%_hash.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_hash.o: src/%/hash.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

$(patsubst %, $(OBJDIR)/%_sign.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_sign.o: src/%/sign.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

$(patsubst %, $(OBJDIR)/%___asm_field.o, $(notdir $(VARIANTS))): $(OBJDIR)/%___asm_field.o: src/%/__asm_field.S
	$(CC) $(CFLAGS) -c $< -o $@

# Compilation rules for test source files
$(patsubst %, $(OBJDIR)/%_test_aim2.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_test_aim2.o: src/%/tests/test_aim2.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

$(patsubst %, $(OBJDIR)/%_test_sign.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_test_sign.o: src/%/tests/test_sign.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

$(patsubst %, $(OBJDIR)/%_test_speed.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_test_speed.o: src/%/tests/test_speed.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@

# Compilation rule for KAT source file per variant
$(patsubst %, $(OBJDIR)/%_PQCgenKAT_sign.o, $(notdir $(VARIANTS))): $(OBJDIR)/%_PQCgenKAT_sign.o: common/PQCgenKAT_sign.c
	$(CC) $(CFLAGS) -I src/% -c $< -o $@
